...
const CODES = [
  ...
  { id: 50, category: "Gas", title: "Gas water heater installation — full walkthrough", titleEs: "Instalación de calentador de agua a gas — procedimiento completo", code: "IPC 501-504 · NFPA 54 · IRC G2427", plain: "Installing a gas water heater correctly from start to finish. This is the most common job a Texas plumber does.\n\nSTEP 1 — SHUTOFFS AND SAFETY:\n• Turn off gas at the shutoff valve within 6 feet of the unit\n• Turn off cold water supply\n• Connect a hose to drain valve and drain the old tank\n• On a gas unit — never use a torch near an old unit that may have residual gas\n\nSTEP 2 — REMOVE OLD UNIT:\n• Disconnect gas line at union or flex connector\n• Disconnect water lines at unions\n• Disconnect vent connector at draft hood\n• Note: old vent connectors may contain rust scale — do not let it fall into the new flue collar\n\nSTEP 3 — INSTALL NEW UNIT:\n• Position new unit — ensure adequate clearances (18 inches from combustibles on sides, front access clear)\n• If in garage — install on 18-inch platform to elevate ignition source above floor level\n• Connect cold water inlet and hot water outlet — cold to cold, hot to hot\n• Install thermal expansion tank on cold supply\n• Install new dielectric unions where copper meets steel\n\nSTEP 4 — GAS CONNECTION:\n• Install new flexible gas connector — never reuse an old connector\n• Install sediment trap (drip leg) within 12 inches of unit\n• Gas shutoff must be within 6 feet and accessible\n• Pressure test gas line — 10 PSI for 15 minutes minimum in Texas\n\nSTEP 5 — VENTING:\n• Install new single-wall or Type B vent connector — never reuse old single-wall\n• Slope upward toward vent at minimum 1/4 inch per foot\n• Secure all joints with sheet metal screws — minimum 3 per joint\n• Keep single-wall connector under 5 feet in length\n\nSTEP 6 — TPR VALVE AND DISCHARGE:\n• Install new TPR valve if not pre-installed\n• Run 3/4 inch discharge pipe full size to within 6 inches of floor\n• No valves, reducers, or caps on discharge pipe\n\nSTEP 7 — COMMISSION:\n• Restore cold water supply and check all connections for leaks\n• Restore gas and check all gas connections with approved leak detector (never a flame)\n• Light pilot per manufacturer instructions\n• Set thermostat to 120°F — TCEQ requires maximum 120°F at fixtures in most applications", plainEs: "Instalación de calentador de agua a gas — procedimiento completo paso a paso.\n\nPASO 1: Apagar gas y agua, drenar tanque viejo\nPASO 2: Desconectar líneas de gas y agua, quitar conector de ventilación\nPASO 3: Instalar unidad nueva — verificar espacios libres, plataforma de 18 pulgadas en garage\nPASO 4: Conectar agua fría y caliente, instalar tanque de expansión y uniones dieléctricas\nPASO 5: Instalar conector de gas nuevo, trampa de sedimentos, válvula de cierre a menos de 6 pies, prueba de presión\nPASO 6: Instalar ventilación con pendiente correcta, asegurar con tornillos\nPASO 7: Instalar válvula TPR y tubería de descarga a 6 pulgadas del piso, sin tapas ni válvulas\nPASO 8: Revisar fugas, encender piloto, ajustar a 120°F", tags: ["gas water heater", "installation", "how to install", "water heater", "gas", "TPR", "expansion tank", "vent", "sediment trap", "garage", "120 degrees", "step by step"] },
...
export default function App() {
  ...
  const startVoice = () => {
    ...
    r.onresult = (e) => {
      ...
      // 4 — Strip filler words and check if local codes match
      const stripped = heard.replace(/\b(how|do|i|install|a|an|the|to|from|start|finish|properly|what|is|are|show|me|can|you|tell|explain|where|when|why|should|would|could|does|my|for|in|on|at|with|of|and|or|this|that|these|those|please|need|want|help|about)\b/gi, " ").replace(/\s+/g, " ").trim();
      const localMatches = CODES.filter(c => {
        const q = stripped.toLowerCase();
        if (q.length < 3) return false;
        return (lang === "en" ? c.title : c.titleEs).toLowerCase().includes(q) ||
          (lang === "en" ? c.plain : c.plainEs).toLowerCase().includes(q) ||
          c.code.toLowerCase().includes(q) ||
          (c.tags || []).some(tag => stripped.split(" ").some(word => word.length > 3 && tag.toLowerCase().includes(word)));
      });
      ...
      // 5 — No local match — open YouTube with the full question as a plumbing search
      const ytQuery = lang === "en" ? `plumbing ${heard}` : `plomeria ${heard}`;
      setVoiceStatus(lang === "en" ? `Opening YouTube for: "${heard}"` : `Abriendo YouTube: "${heard}"`);
      setTimeout(() => {
  ...
  return (
    <div style={{ fontFamily: "'Georgia',serif", background: "#0e1215", minHeight: "100vh", maxWidth: 430, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", opacity: 0.08, pointerEvents: "none", zIndex: 0, width: 360, height: 360, maxWidth: "85vw" }}>
        <svg viewBox="0 0 500 500" fill="#d4820a" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", height: "100%" }}>
          <path d="
            M 195 25
            L 195 30 L 196 40 L 195 55 L 196 70 L 195 85
            L 195 100 L 196 115 L 195 130 L 195 155
            L 185 157 L 178 162 L 183 167 L 190 169
            L 200 170 L 218 172 L 240 170 L 265 173
            L 295 171 L 325 174 L 355 172 L 385 175
            L 412 173 L 438 177 L 458 175
            L 472 180 L 478 192 L 478 210
            L 478 235 L 478 260 L 478 285
            L 478 310 L 478 335 L 478 358
            L 476 375 L 470 390 L 460 404
            L 447 417 L 432 429 L 415 440
            L 396 449 L 375 455 L 354 458
            L 336 457 L 321 450 L 310 441
            L 300 450 L 287 457 L 272 460
            L 256 457 L 241 449 L 228 455
            L 213 462 L 196 464 L 179 458
            L 163 447 L 148 433 L 134 417
            L 120 399 L 108 379 L 97 358
            L 88 336 L 81 313 L 76 289
            L 73 265 L 72 241 L 73 218
            L 77 198 L 84 181 L 95 168
            L 110 159 L 128 154 L 148 151
            L 163 149 L 174 144 L 181 135
            L 186 122 L 189 107 L 191 90
            L 191 73 L 191 56 L 193 42
            L 193 32 L 195 25 Z
          " />
        </svg>
      </div>
