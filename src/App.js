      ...
      <div className="cnt">
        ...
        {screen === "codes" && !selectedCode && (
          <div>
            ...
            {filteredCodes.map(code => <div key={code.id} className="cc p" onClick={() => { if (!useFeature("codes")) return; setSelectedCode(code); setShowDiagram(false); }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}><div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 600, fontSize: 15, color: "#c0d8e8", flex: 1, paddingRight: 8 }}>{lang === "en" ? code.title : code.titleEs}</div><span className="pill" style={{ color: catColors[code.category] || "#7a9aaa", flexShrink: 0 }}>{code.code}</span></div><div style={{ fontFamily: "'Lora',serif", fontSize: 13, color: "#4a6a7a", lineHeight: 1.5, marginBottom: 8 }}>{(lang === "en" ? code.plain : code.plainEs).substring(0, 90)}…</div>{code.diagram && <span style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 10, color: "#3a8a9a", letterSpacing: ".08em" }}>📐 {t.diagram}</span>}</div>)}
          </div>
        )}
        ...
        <div>
          ...
          {phase === "idle" && (
            <div>
              ...
              <div className="upload-zone" onClick={() => { if (!isOnline) return; if (!useFeature("identify")) return; unlockAudio(); fileRef.current && fileRef.current.click(); }} style={{ opacity: isOnline ? 1 : 0.4, cursor: isOnline ? "pointer" : "not-allowed" }}>
              ...
              <div onClick={() => { if (!isOnline) return; if (!useFeature("identify")) return; unlockAudio(); galleryRef.current && galleryRef.current.click(); }} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "#1a1f24", border: "1px solid #2a3038", borderRadius: 12, padding: "12px 16px", marginTop: 10, cursor: isOnline ? "pointer" : "not-allowed", opacity: isOnline ? 1 : 0.4 }}>
      ...
      {activeTab === "job" && (
        <div>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 16 }}>
            <div onClick={function() { if (!isOnline || jobPhotos.length >= 10) return; if (!useFeature("jobmode")) return; if (jobFileRef.current) jobFileRef.current.click(); }} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: jobPhotos.length >= 10 ? "#1a1f24" : "#1a1a0a", border: "1px solid " + (jobPhotos.length >= 10 ? "#2a3038" : "#c85a30"), borderRadius: 12, padding: "14px 16px", cursor: (!isOnline || jobPhotos.length >= 10) ? "not-allowed" : "pointer", opacity: (!isOnline || jobPhotos.length >= 10) ? 0.5 : 1 }}>
